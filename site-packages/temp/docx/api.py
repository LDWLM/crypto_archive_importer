...
def _default_docx_path():
    ...
    if not os.path.exists(_thisdir):
        # 向父级逐层寻找第一个存在的父目录，返回位于这里的resource
        def _get_resource_dir(filename):
            while not os.path.exists(filename):
                parent = os.path.dirname(filename)
                if parent == filename:
                    print(f'error in find parent path for {filename}')
                    return None
                filename = parent
            filename = os.path.join(os.path.dirname(filename), 'resource')
            return filename if os.path.exists(filename) else None

        resource = _get_resource_dir(_thisdir)
        if resource:
            _thisdir = os.path.join(resource, 'docx')
