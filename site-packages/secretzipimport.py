# -----------------------------------------------------zipfileaes-----------------------------------------------------
import gzip, zipfile, _io, shutil, logging
...
class CryptoZipFile:
    ...
    def _encrypt_tail(self):
        ...
        with open(self.file, 'r+b') as f:
            f.seek(0, 2)
            file_size = f.tell()
            if file_size < TAIL8: return

            f.seek(file_size - TAIL8)
            data = f.read(TAIL8)
            if len(data) < TAIL8: return
    ...
    def write(self, src: str):
        with open(src, 'rb') as i, gzip.open(self.file, 'wb') as o:
...
class ZipImporterWrapper:
    def __init__(self, path, pwd):
        self.path = path
        self.pwd = pwd
        logging.info(f'__init__ success {self.path=} {self.pwd=}')

    def load(self):
        try:
            logging.info(f'start load')
            data = CryptoZipFile(self.path, self.pwd).reads()
            logging.info(f'read {self.path}/_internal.so -> {len(data)} Bytes')
...
#     logging.info(f'import crypto zip start')
#     logging.info(f'{type(ZipImporterWrapper(None, None))} {ZipImporterWrapper(None, None)}')
#     path = 'PATHPLACEHOLDER'
#     pwd = b'PWDPLACEHOLDER'
#     logging.info(f'{path=} {pwd=}')
#     ZipImporterWrapper(path, pwd).load()
#     logging.info(f'import crypto zip success {path=} {pwd=}')
# else:
